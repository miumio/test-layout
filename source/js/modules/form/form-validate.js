...
const simpleLimitations = (formElement, dataLimitations) => {
    ...
    console.error(
        `Переданный формат ограничения: ${dataLimitations}, не поддерживается. Проверьте корректность введённых значений в соответствии со спецификацией.`
    );
};
...
const simpleMatrix = (formElement, dataMatrix, dataMatrixLimitations) => {
    if (dataMatrixLimitations === null) {
        // eslint-disable-next-line no-console
        console.error(
            'При валидации по матрице обязательно указывать формат ограничений: data-matrix-limitations=""'
        );
        return;
    }
    ...
    if (RegEx) {
        ...
        // eslint-disable-next-line no-console
        console.error(
            `Переданный формат ограничения: ${dataMatrixLimitations}, не поддерживается. Проверьте корректность введённых значений в соответствии со спецификацией.`
        );
    }
};
...
const onKeydownPhoneInput = (e) => {
    if (
        (e.target.selectionStart === 1 && e.keyCode === 8) ||
        e.keyCode === 46
    ) {
        e.preventDefault();
    }
...
const formElementLimitationsAction = (formValidateElement) => {
    const dataLimitations = formValidateElement.dataset.limitations
        ? formValidateElement.dataset.limitations
        : null;
...
const formElementMatrixAction = (formValidateElement) => {
    const dataMatrix = formValidateElement.dataset.matrix
        ? formValidateElement.dataset.matrix
        : null;
    const dataMatrixLimitations = formValidateElement.dataset.matrixLimitations
        ? formValidateElement.dataset.matrixLimitations
        : null;
    let formElement = formValidateElement.querySelector('input');
    if (!formElement) {
        formElement = formValidateElement.querySelector('textarea');
...
const onFormSubmit = (e, callback) => {
    ...
    const groupsFormElement = e.target.querySelectorAll(
        '[data-validate-type="toggle-group"]'
    );
    if (
        showErrors(formElements, groupsFormElement) &&
        callback &&
        callback.validationSuccessCallback
    ) {
...
export default class FormsValidate {
        ...
        if (
            ...
            formWrappers.forEach((wrapper) => {
                if (wrapper.classList.contains('is-initialized')) {
                    // eslint-disable-next-line no-console
                    console.error(
                        'На данной форме валидация уже инициализированна'
                    );
                } else {
            ...
            typeOfNode === '[object HTMLDivElement]' ||
            typeOfNode === '[object HTMLElement]'
        ) {
    ...
    initItem(element) {
        ...
        const formValidateElements = form.querySelectorAll(
            '[data-validate-type]'
        );
        const formLimitationsElements = form.querySelectorAll(
            '[data-limitations]:not([data-validate-type])'
        );
        const formMatrixElements = form.querySelectorAll(
            '[data-matrix]:not([data-validate-type])'
        );
