const MoveTo = (() => {
    ...
    MoveTo.prototype.registerTrigger = function (dom, callback) {
        ...
        const href =
            dom.getAttribute('href') || dom.getAttribute('data-target');
...
class NavigationChanger {
    constructor() {
        ...
        this._blockShift = 50;
        this._linkElements = document.querySelectorAll(
    ...
    _changeLinksActiveState() {
        this._linkElements.forEach((link, index) => {
            ...
            if (!currentBlockElement) {
                ...
                console.error(
                    'Блока на который ссылается ссылка не существует! Проверьте соответствие href ссылок с id блоков'
                );
                return;
            }
            if (
                currentBlockElement.getBoundingClientRect().bottom >
                    this._blockShift &&
                currentBlockElement.getBoundingClientRect().top <=
                    this._blockShift
            ) {
